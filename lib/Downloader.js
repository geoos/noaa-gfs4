...
class Downloader {
    ...
    getState() {        
        try {
            let j = fs.readFileSync(config.dataPath + "/state.json");
    ...
    setState(state) {
        fs.writeFileSync(config.dataPath + "/state.json", JSON.stringify(state));
    }
    init() {
        ...
        try {
            ...
            if (!mExec) {
                ...
                while (hh <= 384) {
                    ...
                    let fileName = forecastTime.format("YYYY-MM-DD_HH-mm") + ".grb2"
